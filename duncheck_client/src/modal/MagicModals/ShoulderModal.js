...
import '../../css/MagicModal/ShoulderModal.css'
    ...
    return (
        <div className = {open ? 'openShoulderModal shoulderModal' : 'shoulderModal'}>
            {open ? (
                <MagicCutBox>
                    <MagicInput>
                        <MagicCutLabel>머리어깨 : </MagicCutLabel>
                        <MagicCutValue type = 'number' onChange = {onHandlerShoulder} value = {shoulder}/>
                        <SettingMagicButton className = 'settingShoulder' onClick = {() => {onSubmitShoulder(); close();}}>닫기</SettingMagicButton>
                    </MagicInput>
                </MagicCutBox>
            ): null}
        </div>
